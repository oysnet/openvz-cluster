...
Count.prototype.__doCheckCount = function() {
	...
	if (runnings.length > this.number) {

		log.info('Check running container for class ' + this.clazz.type
				+ ', need to increase container count by '
				+ (runnings.length - this.number))
		...
		for (var i = 0, l = runnings.length - this.number; i < l; i++) {
			...
			asyncMethods.push(function(container, callback) {
						log
								.notice('stopping running container '
										+ container.id);

						container.start(function(err, res) {
									// todo: what if a container fail to start
									if (err !== null) {
										log.error('Container fail to start '
												+ container.id)
									}

									log.notice('Container stopped '
											+ container.id)
		...
		log.info('Check running container for class ' + this.clazz.type
				+ ', need to decrease container count by '
				+ (this.number - runnings.length))
		...
		for (var i = 0, l = this.number - runnings.length; i < l; i++) {
			...
			if (spares.length > 0) {
				...
				asyncMethods.push(function(container, callback) {
							log.notice('starting spare container '
									+ container.id);
							container.start(function(err, res) {
										...
										if (err !== null) {
											log
													.error('Spare container fail to start '
															+ container.id)
										}

										log.notice('Spare container started '
												+ container.id)
				...
				asyncMethods.push(function(callback) {
							...
							console.log(availlableHosts.length)
							...
							if (host === null) {
								log.warning('No host availlable to create new container');
								callback();
								return;
							}
							...
							container.setup(function(err) {
										...
										if (err !== null) {
											this.__failedCreateOnHosts
													.push(host);
											log
													.error('Container fail to setup '
															+ container.id)
										}
										log
												.notice('Container set up and started '
														+ container.id)
										callback();
									}.bind(this));
						}.bind(this));
		...
		log.info('Check running containers for class ' + this.clazz.type
				+ ' OK')
		this.__checking = false;
	}
