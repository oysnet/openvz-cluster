...
HA.prototype.__computeStatus = function() {
  ...
  if(this.status  === HA.STATUS.UP) {
    log.info("CLUSTER STATUS ( "+this.clazz.type+" ) : " + this.status)
  } else {
    log.error("CLUSTER STATUS ( "+this.clazz.type+" ) : " + this.status)
  }
}
...
HA.prototype.__doCheckStatus = function() {
    ...
    this.__computeStatus();
		log.error('CLUSTER ( '+this.clazz.type+' ) : too many masters [' + ctids.join(',') + ']')
		this.__checking = false;
		return;
		...
		async.series([function promoteNewMaster(callback) {
					...
					for (var i = 0, l = deads.length; i < l; i++) {
						asyncList.push(function(deadContainer, callback) {
									deadContainer.removeIp(this.ip, false, function(
													err, res) {
												if (err !== null) {
													log
															.error('CLUSTER ( '+this.clazz.type+' ) : Error removing ip');
												}
												callback();
											})
								}.bind(this, deads[i]));
					...
					newMaster.addIp(this.ip, false, callback);
				}.bind(this)], function(err, callback) {
			this.__checking = false;
			if (err) {
				log.error('CLUSTER ( '+this.clazz.type+' ) : error setting master ' + err);
			}
			this.__checkStatus();
		}.bind(this));
	} else {
    ...
    this.__computeStatus();
		log.error('CLUSTER ( '+this.clazz.type+' ) : no container available')
		this.__checking = false;
	}
}
HA.demote = function(container, callback) {
	container.demote(function(err, res) {
				if (err) {
					log.error('CLUSTER ( '+this.clazz.type+' ) : container refuse demote error is ' + err);
				}
				container.removeIp(this.ip, false, callback);
			}.bind(this));
}
...
HA.promote = function(container, callback) {
	container.promote(function(err, res) {
				if (err) {
					log.error('CLUSTER ( '+this.clazz.type+' ) : container refuse promote error is ' + err);
				}
				container.addIp(this.ip,false, callback);
			}.bind(this));
}
