...
Container.prototype.setup = function(callback) {
  ...
  async.series({
    ...
    'set_hostname' : function(callback) {
        this.host.vzctl(['set',this.id,'--hostname', [this.id ,this.hostname].join('.'),'--save'], null, logCallback.bind(this,callback,'Set hostname'));
      }.bind(this),
    ...
    'set_ip' : function(callback) {
        if(this.network === 'venet') {
          ...
          async.series([
            ...
            function(callback) {
              this.exec('echo "auto eth0">>/etc/network/interfaces',null,callback);
            }.bind(this),
            function(callback) {
              this.exec('echo "iface eth0 inet static">>/etc/network/interfaces',null,callback);
            }.bind(this),
            function(callback) {
              this.exec('echo "address '+this.ip+'">>/etc/network/interfaces',null,callback);
            }.bind(this),
            function(callback) {
              this.exec('echo "broadcast 10.7.50.255">>/etc/network/interfaces',null,callback);
            }.bind(this),
            function(callback) {
              this.exec('echo "netmask 255.255.255.0">>/etc/network/interfaces',null,callback);
            }.bind(this),
            function(callback) {
              this.exec('echo "up ip route add 10.7.35.254 dev eth0  scope link">>/etc/network/interfaces',null,callback);
            }.bind(this),
            function(callback) {
            	
              this.exec('echo "up ip route add default via 10.7.35.254 dev eth0">>/etc/network/interfaces',null,callback);
            }.bind(this),
            this.stop.bind(this),
