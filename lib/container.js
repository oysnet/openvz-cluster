...
Container.prototype.addIp = function(ip, type, callback) {
	if (type !== Container.NETWORK.VENET) {
		throw "Can't add ip for this type of network";
	}
	...
	async.series({
		...
		'init_data' : function(callback) {
			if (this.hostname === undefined) {
				this.hostname = 'vm'+this.id + '.' + this.constructor.type.toLowerCase() +'.cl.oxys.net';
			}
			...
			this.host
					.vzctl(
							[
									...
									'--ostemplate',
									this.ostemplate
									], null, logCallback.bind(
									this, callback, 'Created new container'));
		}.bind(this),
		...
		'set_ip' : function(callback) {
			if (this.network === Container.NETWORK.VENET) {
				...
				this.addIp(this.ip, Container.NETWORK.VENET, logCallback.bind(this, callback,
								'Configure venet device with ip ' + this.ip[0])); //todo multiple ip
			} else {
				...
				async.series([function(callback) {
					...
					this
							.exec(
									...
									null, callback);
				}.bind(this), this.stop.bind(this)], logCallback
						.bind(this, callback, 'Configure veth device with ip '
										+ this.ip[0]));
			}
		}.bind(this),
