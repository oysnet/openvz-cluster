...
function Container(info, host) {
	...
	this.network = 'venet';
	this.alive = null;
	this.monitorFrequency = 500;
	this.__monitorId = null;
...
util.inherits(Container, process.EventEmitter);

Container.prototype.refreshInfo = function (info) {
	var attrs = ['host', 'id', 'ip', 'swap', 'ram', 'status', 'hostname'];
	for (var i = 0, l = attrs.length; i<l; i++) {
			...
			if (old !== value) {
				this.emit('change' + attrs[1][0].toUpperCase() + attrs[1].substr(1), value, old)
			}
...
Container.prototype.isMonitoring = function () {
	return this.__monitorId !== null;
}


Container.prototype.startMonitoring = function () {
	this.__monitorId = setInterval(this._monitor.bind(this), this.monitorFrequency);
}

Container.prototype.addListener = function (event, listener) {
	
	process.EventEmitter.prototype.addListener.call(this, event, listener);
	
	if ( event === 'changeAlive' && this.isMonitoring() === false) {
...
Container.prototype._monitor = function () {
	this.isAlive(function (err, alive) {
		
		var old = this.alive;
		this.alive = (err === null && alive === true);
	 
		if (old !== this.alive) {
			console.log('emit')
			this.emit('changeAlive', this.alive, old);
		}
