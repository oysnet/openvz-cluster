...
Container.prototype.refreshInfo = function(info) {
	...
	for (var i = 0, l = attrs.length; i < l; i++) {
		...
		if (old instanceof Array && value instanceof Array) {

			old.sort();
			value.sort();
			if (old.toString() !== value.toString()) {
				this.emit('change' + attrs[i][0].toUpperCase()
								+ attrs[i].substr(1), value, old)
	...
	var old = this.status;
	var value = (info.vmStatus === Container.VMSTATUS.RUNNING && (this.ipMonitor !== null
			? this.ipMonitor.status === IpMonitor.STATUS.UP
			: null)) ? Container.STATUS.UP : Container.STATUS.DOWN;
...
Container.prototype.addListener = function(event, listener) {
	...
	if (event === 'changeStatus') {
		if (this.ipMonitor === null) {
			this.ipMonitor = new IpMonitor(this.ip[0]);
	...
	async.series({
		...
		'init_data' : function(callback) {
			...
			if (this.ip === undefined) {
				...
				var net = this.host.defaultNetworkAddress.split('.');
				net.splice(3,1,this.id);
				this.ip = [net.join('.')];
			}
		...
		'set_ram' : function (callback) {
			
			this.setRam(this.ram,  logCallback.bind(this,
										callback,
										'Set RAM to '
												+ this.ram))
		...
		'set_nameserver' : function(callback) {
			this.host.vzctl(['set', this.id, '--nameserver', this.host.nameServerAddress,
							'--save'], null, logCallback.bind(this, callback,
							'Set nameserver'));
		}.bind(this),
