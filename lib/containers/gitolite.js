...
Gitolite.prototype.setup = function(callback) {
  console.log('Setup gitolite …');
  Container.prototype.setup.apply(this,[function(err,res) {
      if(err) {
        console.log ('creation error', err);
        return;
      }
      this.installGitolite(callback);
  }.bind(this)]);
}
...
Gitolite.prototype.installGitolite = function(callback) {
     ...
     this.exec( [
        ...
        'ssh -o "StrictHostKeyChecking no" git@localhost', // add key finger print
        ...
        'cd /root/gitolite-admin; git config --global user.name "System";git config --global user.email admin@oxys.net; git commit -am "initial setup"; git push origin master'
          ...
          this.description.type='gitolite';
...
Gitolite.prototype.addRepository = function (name, callback) {
	this.exec([
		...
		'echo "    	RW+ = admin @admins @'+name+'" >> conf/gitolite.conf',
...
Gitolite.prototype.addUserInGroup = function (user, group, callback) {
	this.exec([
		'cd /root/gitolite-admin/',
		'git pull',
		'cat conf/gitolite.conf | sed -e "s/@'+group+' =/@'+group+' = '+user+'/" > conf/gitolite.conf~',
		'mv conf/gitolite.conf~ conf/gitolite.conf',
		'git commit -am "add user '+ user +' in group '+ group +'"',
		'git push origin master'
	].join(' && '), null,callback);
}
