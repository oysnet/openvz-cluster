...
function HASupervisor(cluster, clazz, ip) {
...
HASupervisor.prototype.__computeStatus = function() {
  console.log(this.ipMonitor.status, IpMonitor.STATUS.UP ,this.masterStatus , HASupervisor.STATUS.UP,'---')
  if(this.ipMonitor.status === IpMonitor.STATUS.UP && this.masterStatus === HASupervisor.STATUS.UP) {
    this.status = HASupervisor.STATUS.UP;
  } else if(this.masterStatus === HASupervisor.STATUS.ERROR ||(this.ipMonitor.status === IpMonitor.STATUS.DOWN && this.masterStatus === HASupervisor.STATUS.UP )) {
    this.status = HASupervisor.STATUS.ERROR;
  } else if(this.ipMonitor.status === IpMonitor.STATUS.DOWN && this.masterStatus === HASupervisor.STATUS.DOWN) {
    this.status = HASupervisor.STATUS.DOWN;
  }
  console.log("CLUSTER STATUS : " + this.status)
}

HASupervisor.prototype.__addContainer = function(container) {
...
HASupervisor.prototype.__removeContainer = function(container) {
	if (container.constructor.type !== this.clazz.type) {
		return;
	}
...
HASupervisor.prototype.__checkStatus = function() {
...
HASupervisor.prototype.__doCheckStatus = function() {
	if (this.__checking) {
		this.__checkStatus();
		return;
	}
	...
	if (masters.length === 1 && masters[0].status === Container.STATUS.UP) {
		this.masterStatus = HASupervisor.STATUS.UP;
    this.__computeStatus();
		...
		this.masterStatus = HASupervisor.STATUS.ERROR;
    this.__computeStatus();
		...
		this.masterStatus = HASupervisor.STATUS.DOWN;
    this.__computeStatus();
		log.error('no container available')
		this.__checking = false;
	}
}
HASupervisor.demote = function(container, callback) {
...
HASupervisor.promote = function(container, callback) {
...
exports.HASupervisor = HASupervisor;
