var SSHHost = require('./sshHost').SSHHost, Container = require('./container').Container, htmlDecode = require('./encoder').htmlDecode, log = require('czagenda-log')
		.from(__filename), path = require('path'), errors = require('./errors'), ContainerTypes = require('./containerTypes');
...
Host.prototype = new process.EventEmitter();
...
Host.prototype._refreshContainers = function() {
	...
	h.ssh(cmd, null, function(err, res, code) {
		...
		for (var i = 0, l = res.length; i < l; i++) {
			this._constructContainer(res[i])
		...
		for (var i = 0, l = this.containers.length; i < l; i++) {
			if (containers.indexOf(this.containers[i].id) === -1) {
				...
				this.emit('removeContainer', this.containers.splice(i, 1));
			}
		}
			// TODO event ?
	}.bind(this));
 ...
 * 
 * if (typeof(module.type) == 'undefined') { throw new Error('Class must define
 * a static attribute type') }
 * 
...
Host.prototype._updateVzinfo = function(callback) {
	var h = new SSHHost(this.ip, this.user);
	h.scpTo(path.join(path.dirname(__filename), '../ressources/vzinfo.py'),
			'/root/', callback);
}
...
Host.prototype.getContainersByType = function(clazz, strict) {
	var results = [];
	for (var i = 0, l = this.containers.length; i < l; i++) {
		if (strict === true && this.containers[i].type === clazz.type) {
