var spawn = require('child_process').spawn, fs = require('fs'), log = require('czagenda-log')
		.from(__filename);
...
SSHHost.prototype.ssh = function(command, options, callback) {
	var ssh, stderr = '', stdout = '', compress = null, stdout_compressed = null, debug = true;
	...
	ssh.stdout.on('data', function(out) {
				...
				if (stdout instanceof fs.WriteStream) {
					if (compress !== null) {
						try {
							stdout.write(compress.deflate(out),
									encoding = 'binary');
						} catch (e) {
